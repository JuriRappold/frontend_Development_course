document.addEventListener("DOMContentLoaded", () => {
    ...
    const cart = [1, 2, 3, 4];// { name: string, price: number, qty: number }
...
// Validate item or throw Error
    function validateItem(item) {
        ...
        } else if(item.price<=0 || item.qty<=0){
            throw Error("Price and qty must be > 0")
        } else if(!Number.isFinite(item.price/item.qty)){
...
// Compute subtotal for up to 3 items, WITHOUT LOOPS
    function subtotalNoLoops() {
        ...
        let subtotal = 0;
        if (cart.length >= 1){
            subtotal+= lineTotal(cart[0])
            subtotal += lineTotal((cart[1]))
            subtotal += lineTotal((cart[2]))
        }
        return subtotal
